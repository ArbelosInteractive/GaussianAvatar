...
if [ ! -f "$path/keypoints.npy" ]; then
  ...
  bash ./run-openpose-bin.sh $path/images
fi

if [ ! -d "$path/masks" ]; then
  echo "Running mask in $path"
  python ./run-sam.py --data_dir $path
  # python scripts/custom/run-rvm.py --data_dir $path
  python ./extract-largest-connected-components.py --data_dir $path
fi

if [ ! -f "$path/poses.npz" ]; then
  python ./run-romp.py --data_dir $path
fi

if [ ! -f "$path/poses_optimized.npz" ]; then
  echo "Refining SMPL..."
  python ./refine-smpl.py --data_dir $path --gender $2 # --silhouette
fi

if [ ! -f "$path/output.mp4" ]; then
  python3 ../visualize-SMPL.py --path $path --gender $2 --pose $path/poses_optimized.npz --headless --fps 1
fi
